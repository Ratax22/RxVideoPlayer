...
<div class="row g-4 mt-4">
    <div class="col-lg-6">
        <div class="card shadow-sm">
            ...
            <div class="card-body">
                <?php
                $total_calculado = ($total_clientes ?? 0);
                $activos = $clientes_activos ?? 0;
                $offline = $clientes_offline ?? 0;
                $inactivos = $total_calculado - $activos - $offline;

                if ($total_calculado === 0) {
                    echo '<p class="text-muted text-center py-4">No hay dispositivos registrados o accesibles para tu rol.</p>';
                } else {
                    echo '<canvas id="estadosChart" height="250"></canvas>';
                }
                ?>
            </div>
        </div>
...
<?php endif; ?>
<!-- Chart.js con debug y seguridad -->
<script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const canvas = document.getElementById('estadosChart');
        if (!canvas) {
            console.error('Canvas #estadosChart NO encontrado. Posible causa: no hay dispositivos o el bloque no se renderizó.');
            return;
        }

        const activos = <?= $activos ?? 0 ?>;
        const inactivos = <?= $inactivos ?? 0 ?>;
        const offline = <?= $offline ?? 0 ?>;

        console.log('Datos reales para torta:', { activos, inactivos, offline, total: activos + inactivos + offline });

        if (activos + inactivos + offline === 0) {
            console.warn('Todos los valores son 0 - no se dibuja la torta');
            canvas.parentElement.innerHTML += '<p class="text-muted text-center py-4">Sin datos para mostrar.</p>';
            return;
        }
        ...
        new Chart(canvas.getContext('2d'), {
            ...
            options: {
                ...
                plugins: {
                    ...
                    title: { display: true, text: 'Estados actuales', color: '#333' }
                },
                cutout: '60%'
            }
        });
        console.log('Gráfico torta creado correctamente');
    });
</script>